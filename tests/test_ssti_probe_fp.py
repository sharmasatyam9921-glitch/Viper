...
def test_ssti_real_evaluation_true_positive_still_fires():
    """A genuine template engine evaluates the expression and CONSUMES it.

    The injection `${7*7}` is replaced by `49` (the literal `${7*7}` is GONE
    from the body), and the same holds for the corroborating pairs
    `${8*8}`->64 and `${9*9}`->81. Controls (operator mutated) reflect the raw
    text and never yield the evaluated numbers. The worker must STILL flag.
    """

    # (operator-intact substring -> evaluated number)
    _EVAL = {"7*7": "49", "8*8": "64", "9*9": "81"}

    async def fake(method, url, *, headers=None, timeout=10.0, **kw):
        injected = _injected_value(url)
        # Control payloads carry the mutated operator 'NxN' -> reflected as-is,
        # never evaluated.
        for raw, num in _EVAL.items():
            n1, n2 = raw.split("*")
            if f"{n1}x{n2}" in injected:
                return HttpResp(200, {"content-type": "text/html"},
                                f"<p>echo: {injected}</p>", url)
        # Live payloads: a real engine evaluates and the literal expression is
        # consumed (replaced by the number, the '${...}' wrapper is gone).
        body = injected
        for raw, num in _EVAL.items():
            if raw in body:
                # Strip the whole template wrapper around this expression and
                # substitute the evaluated number -> payload is CONSUMED.
                body = re.sub(r"[\$#{<%=]*\s*" + re.escape(raw) + r"\s*[}>%]*",
                              num, body)
        return HttpResp(200, {"content-type": "text/html"},
                        f"<p>rendered: {body}</p>", url)
