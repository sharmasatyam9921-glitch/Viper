...
interface UseTrufflehogSSEReturn {
  logs: ReconLogEvent[]
  isConnected: boolean
  error: string | null
  clearLogs: () => void
...
export function useTrufflehogSSE({
  ...
  onComplete,
  onError,
}: UseTrufflehogSSEOptions): UseTrufflehogSSEReturn {
  ...
  const connect = useCallback(() => {
    ...
    eventSource.addEventListener('log', (event) => {
      try {
        ...
        console.error('Error parsing trufflehog SSE log event:', err)
      }
    })
    ...
    eventSource.addEventListener('error', (event) => {
      try {
        ...
        console.error('Error parsing trufflehog SSE error event:', err)
      }
    })
    ...
    eventSource.addEventListener('complete', (event) => {
      try {
        ...
        console.error('Error parsing trufflehog SSE complete event:', err)
      }
    })
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        console.error('Error parsing trufflehog SSE message:', err)
      }
    }
...
export default useTrufflehogSSE
