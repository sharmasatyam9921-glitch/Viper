...
interface UseGithubHuntSSEOptions {
...
interface UseGithubHuntSSEReturn {
  logs: ReconLogEvent[]
  isConnected: boolean
  error: string | null
  clearLogs: () => void
...
export function useGithubHuntSSE({
  projectId,
  enabled,
  onLog,
  onPhaseChange,
  onComplete,
  onError,
}: UseGithubHuntSSEOptions): UseGithubHuntSSEReturn {
  ...
  const connect = useCallback(() => {
    ...
    const eventSource = new EventSource(`/api/github-hunt/${projectId}/logs`)
    ...
    eventSource.addEventListener('log', (event) => {
      try {
        ...
        console.error('Error parsing GitHub hunt SSE log event:', err)
      }
    })
    ...
    eventSource.addEventListener('error', (event) => {
      try {
        ...
        console.error('Error parsing GitHub hunt SSE error event:', err)
      }
    })
    ...
    eventSource.addEventListener('complete', (event) => {
      try {
        ...
        console.error('Error parsing GitHub hunt SSE complete event:', err)
      }
    })
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        console.error('Error parsing GitHub hunt SSE message:', err)
      }
    }
...
export default useGithubHuntSSE
